...
> = {
  ...
  Active: {
    next: "Closed",
    label: "Close cycle",
    description: "Reviewers will no longer be able to submit or edit their responses.",
  },
  Closed: {
    next: "Published",
...
function NewTemplateDialog({
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<ReviewTemplateFormValues>({ resolver: zodResolver(reviewTemplateSchema), defaultValues });

  const { fields, append, remove } = useFieldArray({ control, name: "questions" });
  const questions = watch("questions");
  const totalWeight = questions?.reduce((sum, q) => sum + (Number(q.weight) || 0), 0) ?? 0;
    ...
    >
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Score = Σ(Rating×Weight)/ΣWeight — weights just need to reflect relative
            importance, they don't need to add up to any particular number.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
          <div className="space-y-1.5">
            ...
            <Input id="template-name" placeholder="e.g. Engineering — Mid-year 2026" {...register("name")} />
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-xs text-muted-foreground">Total weight: {totalWeight}</span>
            </div>
              ...
              <div key={field.id} className="flex gap-2 rounded-md border border-border p-3">
                ...
                <div className="w-20 space-y-1.5">
                  ...
                  {errors.questions?.[index]?.weight && (
                    <p className="text-xs text-destructive">{errors.questions[index]?.weight?.message}</p>
                  )}
                </div>
                <button
                  type="button"
            ...
            <Button type="button" variant="outline" size="sm" onClick={() => append({ question_text: "", weight: 1 })}>
    ...
    <div className="space-y-4">
      ...
      {!templates?.length ? (
        <div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground">No templates yet — create one before launching a cycle.</p>
        </div>
      ) : (
          ...
          {templates.map((template) => (
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {template.questions.length} question{template.questions.length === 1 ? "" : "s"} · total
                weight {template.questions.reduce((sum, q) => sum + q.weight, 0)}
              </p>
            </div>
          ))}
        </div>
...
function NewCycleDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<ReviewCycleFormValues>({ resolver: zodResolver(reviewCycleSchema) });
      ...
      <DialogContent>
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          <DialogHeader>
            ...
            <DialogDescription>Starts in Draft — nothing happens until you launch it.</DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label htmlFor="cycle-name">Cycle name</Label>
              <Input id="cycle-name" placeholder="e.g. H1 2026 Performance Review" {...register("name")} />
              {errors.name && <p className="text-xs text-destructive">{errors.name.message}</p>}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              />
              {errors.template_id && <p className="text-xs text-destructive">{errors.template_id.message}</p>}
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label htmlFor="cycle-start">Start date</Label>
                <Input id="cycle-start" type="date" {...register("start_date")} />
                ...
                return (
                  <tr key={cycle.id}>
                    ...
                    <td className="px-4 py-3">
                      <span className={statusPillClass(cycle.status)}>{cycle.status}</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      {cycleTransition && (
                        <Button size="sm" variant="outline" onClick={() => setConfirmCycle(cycle)}>
