...
import { useVerifyActivationToken, useActivateAccount, useResendActivation } from "./onboardingApi";
      ...
      <div className="relative hidden w-[42%] flex-col justify-between overflow-hidden bg-primary px-12 py-10 text-primary-foreground lg:flex">
        ...
        <div className="max-w-sm">
          ...
          <p className="mt-4 text-sm leading-relaxed text-primary-foreground/70">
            Set a password to finish activating your account and get into your profile.
          </p>
        </div>
        <div />
      </div>
...
function ResendActivationForm({ prefillEmail }: { prefillEmail?: string }) {
  ...
  if (sent) {
    return (
      <div className="flex items-start gap-2 rounded-md bg-success/10 px-3 py-2.5 text-sm text-success">
        ...
        <span>If that email has a pending invite, a new activation link is on its way.</span>
      </div>
    );
  }
  ...
  return (
    <form
      onSubmit={handleSubmit((values) => resendMutation.mutate(values, { onSuccess: () => setSent(true) }))}
      className="space-y-3"
      noValidate
    >
      <div className="space-y-1.5">
        <Label htmlFor="resend-email">Work email</Label>
        <Input id="resend-email" type="email" placeholder="you@company.com" {...register("email")} />
        {errors.email && <p className="text-xs text-destructive">{errors.email.message}</p>}
      </div>
      <Button type="submit" variant="outline" className="w-full" disabled={resendMutation.isPending}>
...
export default function ActivateAccountPage() {
  ...
  const { data: verification, isLoading: isVerifying, isError: verifyFailed } =
    ...
    return (
      <BrandPanel>
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <h2 className="font-display text-xl font-semibold text-foreground">Missing activation link</h2>
          <p className="text-sm text-muted-foreground">
            This page needs the link from your invitation email. If you've lost it, request a new one below.
          </p>
        </div>
        <div className="mt-6">
    ...
    return (
      <BrandPanel>
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Activation links are only valid for a limited time. Enter your email and we'll send a fresh one.
          </p>
        </div>
        <div className="mt-6">
          <ResendActivationForm />
  ...
  return (
    <BrandPanel>
      ...
      <p className="mt-1.5 text-sm text-muted-foreground">
        {verification.full_name ? `Hi ${verification.full_name.split(" ")[0]}, choose` : "Choose"} a
        password for {verification.email ?? "your account"}.
      </p>
